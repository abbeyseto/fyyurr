...
def search_venues():
    ...
    venue_query_city = Venue.query.filter(Venue.city.ilike(
        '%' + request.form['search_term'] + '%'))
...
def create_venue_submission():
    try:
        new_venue = Venue(
            ...
            seeking_talent=str2bool(request.form['seeking_talent']),
            description=request.form['description'],
        )
        print(new_venue.detail())
...
def delete_venue(venue_id):
    ...
    try:
        ...
        flash("There was an error deleting this Venue")
        db.session.rollback()
    finally:
        db.session.close()
...
def show_artist(artist_id):
    ...
    if artist_query:
        ...
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        new_shows_query = Show.query.options(db.joinedload(Show.Artist)).filter(
        ...
        past_shows_query = Show.query.options(db.joinedload(Show.Artist)).filter(
            Show.artist_id == artist_id).filter(Show.start_time <= current_time).all()
...
def edit_artist_submission(artist_id):
    ...
    if artist_data:
        ...
        setattr(artist_data, 'seeking_venue',
                str2bool(request.form['seeking_venue']))
...
def edit_venue(venue_id):
    ...
    if venue_query:
        ...
        form.description.data = venue_details["description"]
...
def edit_venue_submission(venue_id):
    ...
    if venue_data:
        ...
        setattr(venue_data, 'description', request.form['description'])
        setattr(venue_data, 'seeking_talent', str2bool(
...
def create_artist_submission():
    ...
    try:
        ...
        new_artist = Artist(
            ...
            image_link=request.form['image_link'],
            facebook_link=request.form['facebook_link'],
            seeking_venue=str2bool(request.form['seeking_venue']),
            seeking_description=request.form['seeking_description'],
        )
...
def delete_artist(artist_id):
    ...
    try:
        ...
        db.session.commit()
        flash("The artist has been deleted successfully")
    except:
        flash("An error occured, The artist has not been deleted successfully")
        db.session.rollback()
    finally:
        db.session.close()
    # BONUS CHALLENGE: Implement a button to delete a Venue on a Venue Page, have it so that
